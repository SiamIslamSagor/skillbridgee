...
import { clamp, springIsSettled, stepSpring, type SpringState } from "./spring";
...
class MagneticEngine {
  ...
  private loop = () => {
    ...
    for (const entry of this.entries.values()) {
      ...
      if (this.enabled && hasMoved) {
        const centerX = entry.rect.left + entry.rect.width / 2;
        const centerY = entry.rect.top + entry.rect.height / 2;
        const dx = pointerX - centerX;
        const dy = pointerY - centerY;
        const distance = Math.hypot(dx, dy);
        const force = clamp(1 - distance / entry.radius, 0, 1);

        if (force > 0) {
          let x = dx * force * entry.strength;
          let y = dy * force * entry.strength;
